...
def main(args):
    ...
    for i in range(num_tasks):
        ...
        # Initialize the results container to be saved to file
        if subrank == 0:
        ...
        if subrank == 0:
            ...
            for selection_method in selection_methods:
                ...
                for field in fields:
                    results_dict[selection_method][field][0] = calc_result(X, X_test, y, y_test,
                                                                           beta.ravel(), field,
                                                                           exp_results[selection_method])


            # Save to file. First unravel the index based on rank/task
            task_idx = np.unravel_multi_index((subrank, i), dims = (numproc, num_tasks))

            results_file = '%s/%s.h5' % (results_dir, task_idx)
...
if __name__ == '__main__': 
    ...
    parser.add_argument('results_dir', default = 'results.h5')
