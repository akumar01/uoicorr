...
def FISTA(x0, A, b, alpha, max_iter = 1000, tol=1e-6):
    ...
    # Minimize 1/(2n) * ||A x - b||^2 + \alpha |x|_1, where alpha can be a vector of diff.
    # values
    ...
    p_L = lambda y, A, b, alpha, t: T_lambda(alpha * t, y - 1/b.size * t * A.T @ (A @ y - b))    

    # Used to assess stopping criteria
    loss_fn = lambda y, A, b, alpha: 1/(2 * b.size) * np.linalg.norm(A @ y - b)**2 + alpha @ np.abs(y)
    
    # Calculate the Lipshitz constant associated with this problem 
    # Calculate the largest eigenvalue of the Hessian
    L = 2 * eigh(1/n * A.T @ A, eigvals_only=True, eigvals=(A.shape[1] - 1, A.shape[1] - 1))
    ...
    for k in range(max_iter):
        ...
        # Stopping criteria: Difference in function values should be less than tol        
        if k > 1:
    ...
    return (x[1], final_loss, final_tol, k)
